        ...
        <nav id="mobile-navigation" aria-label="Mobile navigation" className="2xl:hidden border-t border-sky-200/15 border-b border-sky-200/20 bg-[#020b14]/98 shadow-2xl backdrop-blur-xl">
          <div className="mx-auto max-h-[calc(100vh-5rem)] max-w-7xl overflow-y-auto px-4 py-4 sm:px-6">
            <button
              ...
              className="block w-full border-b border-white/10 px-3 py-3 text-left text-xl font-bold text-white hover:bg-cyan-300/10 hover:text-white"
            >
              Dashboard
            </button>
            ...
            {mobileNavGroups.map((group) => (
              <details key={group.title} className="group border-b border-white/10">
                <summary className="flex cursor-pointer list-none items-center justify-between px-3 py-3 text-xl font-bold text-white hover:bg-cyan-300/10 hover:text-white [&::-webkit-details-marker]:hidden">
                  {group.title}
                ...
                <div className="pb-2 pl-3">
                  {group.links.map((link, index) => link.section ? (
                    <div key={`${link.name}-${index}`} className="px-4 pb-1 pt-3 text-lg font-black uppercase tracking-[0.22em] text-white">
                      {link.name}
                    </div>
                  ) : link.href ? (
                    <a
                      ...
                      className="block w-full px-4 py-2.5 text-left text-xl text-white hover:bg-cyan-300/10 hover:text-white"
                    >
                      {link.name}
                    </a>
                  ) : (
