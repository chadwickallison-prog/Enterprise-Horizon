...
const LandingPage: React.FC<LandingPageProps> = ({ onLogin, onSignUp, onForgotPassword, authError, setAuthError, isLoading, onRequestDemo }) => {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try { await onLogin(email, password, rememberMe); } catch (error) {}
  };

  const handleSignUpSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try { await onSignUp({ username, email, password, recoveryPhrase }, rememberMe); } catch (error) {}
  };
          ...
          <form onSubmit={handleSignUpSubmit} className="space-y-4">
            ...
            <div className="flex items-center justify-start text-sm">
                <label className="flex items-center text-gray-400 cursor-pointer">
                    <input type="checkbox" checked={rememberMe} onChange={e => setRememberMe(e.target.checked)} className="h-4 w-4 bg-gray-700 border-gray-600 text-blue-600 focus:ring-blue-500 rounded" />
  ...
  return (
    <div className="text-center animate-fade-in flex flex-col items-center">
        ...
        <p className="mt-2 max-w-2xl mx-auto text-lg text-gray-400">Log in or sign up to access the Readiness Assessment.</p>
        ...
        <div className="mt-8 space-y-4 text-center">
            ...
            <button onClick={onRequestDemo} className="bg-blue-600 hover:bg-blue-700 text-white transition-colors px-6 py-3 rounded-md font-bold shadow-lg">Request a Demo</button>
        </div>
    </div>
  );
};
