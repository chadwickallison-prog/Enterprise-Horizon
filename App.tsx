...
const App: React.FC = () => {
  ...
  const handleSignUp = async (data: SignUpData, remember: boolean) => {
    ...
    try {
      ...
      setAppState('dashboard');
      setCurrentPage('dashboard');
      if (remember) localStorage.setItem(REMEMBERED_EMAIL_KEY, data.email);
    } catch (error) {
      setAuthError((error as Error).message);
    } finally {
      setIsLoading(false);
  ...
  const handleBack = () => {
    ...
    if (page === 'assessment') setAppState('assessment');
  ...
  const renderContent = () => {
    ...
    if (appState === 'page') {
      switch (currentPage) {
        ...
        case 'quantum-communications': return <QuantumCommunicationsPage />;
